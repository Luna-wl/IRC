...
/*   Updated: 2024/01/20 16:23:21 by tkraikua         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Invite::execute(Client * client, std::vector<std::string> &args)
{
	if ( !client->isRegist() ) {
		...
		if ( !user ) {
			client->receive_message(ERR_NOSUCHNICK(_srv->getName(), args[0], user_name));
		} else if ( !channel ) {
			...
			client->receive_message(ERR_USERONCHANNEL(_srv->getName(), args[0], user_name, "#" + channel_name));
		} else {
